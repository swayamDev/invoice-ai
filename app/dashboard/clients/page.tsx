...
function ClientForm({ form, onChange }: ClientFormProps) {
  const field =
    "mt-1.5 bg-[#111] border-white/10 text-white placeholder:text-white/20 focus-visible:ring-0 focus:border-[#FF0A54]/50";
  return (
...
function ActionMenu({ client, onEdit, onDelete }: ActionMenuProps) {
  return (
    <DropdownMenu>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="px-2 py-0.5 rounded-full bg-[#FF0A54]/15 text-[#FF0A54] text-xs font-semibold">
            {clients.length}
          </span>
        </div>
        <Button
      ...
      {loading ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {paginated.map((client) => (
            <div
              key={client.id}
              className="bg-[#0a0a0a] border border-white/8 hover:border-[#FF0A54]/25 rounded-xl p-5 transition-all group"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <Avatar className={`w-10 h-10 ${getColor(client.name)}`}>
                    <AvatarFallback
                      className={`${getColor(client.name)} text-white text-sm font-bold`}
                    >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  {["Client", "Contact", "Company", "Address", "Added", ""].map(
                    (h, i) => (
                      <th
                        key={i}
                    ...
                    <td className="py-3.5 px-5">
                      <div className="flex items-center gap-3">
                        <Avatar className="w-8 h-8">
                          <AvatarFallback
                            className={`${getColor(client.name)} text-white text-xs font-bold`}
                          >
                            {getInitials(client.name)}
                          </AvatarFallback>
                        </Avatar>
                    ...
                    <td className="py-3.5 px-5">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5 text-white/50 text-xs">
                          <RiMailLine className="w-3 h-3 text-[#FF0A54]" />
                          {client.email}
                        </div>
                        {client.phone && (
                          <div className="flex items-center gap-1.5 text-white/50 text-xs">
                            <RiPhoneLine className="w-3 h-3 text-[#FF0A54]" />
                            {client.phone}
                          </div>
                        )}
                      </div>
        ...
        <AlertDialogContent className="bg-[#0a0a0a] border-red-500/20">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/50">
              This will permanently delete this client. Their invoices will
