...
const PER_PAGE = 10
const STATUSES: Array<InvoiceStatus | 'all'> = ['all', 'unpaid', 'paid', 'draft']
...
export default function InvoicesPage() {
  ...
  const updateStatus = async (id: string, status: InvoiceStatus) => {
    const { error } = await supabase.from('invoices').update({ status, ...(status === 'paid' ? { paid_at: new Date().toISOString() } : {}) }).eq('id', id)
  ...
  const deleteInvoice = async () => {
    ...
    if (error) { toast.error('Failed to delete'); setDeleteId(null); return }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Invoiced', value: fmt(stats.total), color: 'text-white' },
          { label: 'Paid', value: fmt(stats.paid), color: 'text-emerald-400' },
          { label: 'Unpaid', value: fmt(stats.unpaid), color: 'text-red-400' },
          { label: 'Total Invoices', value: stats.count.toString(), color: 'text-white' },
        ].map(s => (
          <div key={s.label} className="bg-[#0a0a0a] border border-white/8 rounded-xl p-4">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  {['Invoice', 'Client', 'Date', 'Due', 'Amount', 'Status', ''].map((h, i) => (
                  ...
                  <tr key={inv.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    ...
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-white/80 text-sm truncate max-w-28">{inv.clients?.name || '—'}</span>
                      </div>
                    </td>
                    <td className="py-4 px-5 text-white/50 text-sm">{fmtDate(inv.issue_date)}</td>
                    <td className="py-4 px-5 text-white/50 text-sm">{fmtDate(inv.due_date)}</td>
                    <td className="py-4 px-5 text-white font-medium">{fmt(inv.total, inv.currency)}</td>
                    <td className="py-4 px-5">
                    ...
                    <td className="py-4 px-5 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-[#0d0d0d] border-white/10">
                          ...
                          {inv.status === 'draft' && (
                            <DropdownMenuItem onClick={() => updateStatus(inv.id, 'unpaid')} className="text-white/60 hover:text-white focus:text-white focus:bg-white/5 gap-2 cursor-pointer">
                              <RiSendPlaneLine className="w-4 h-4" /> Mark as Sent
                            </DropdownMenuItem>
                          )}
                          {inv.status === 'unpaid' && (
                            <>
                              ...
                              <DropdownMenuItem onClick={() => updateStatus(inv.id, 'paid')} className="text-white/60 hover:text-white focus:text-white focus:bg-white/5 gap-2 cursor-pointer">
                                <RiCheckLine className="w-4 h-4" /> Mark as Paid
                              </DropdownMenuItem>
                            </>
                          )}
                          <DropdownMenuItem onClick={() => downloadPDF(inv)} className="text-white/60 hover:text-white focus:text-white focus:bg-white/5 gap-2 cursor-pointer">
                            <RiDownloadLine className="w-4 h-4" /> Download PDF
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => duplicateInvoice(inv)} className="text-white/60 hover:text-white focus:text-white focus:bg-white/5 gap-2 cursor-pointer">
                            <RiFileCopyLine className="w-4 h-4" /> Duplicate
                          </DropdownMenuItem>
                          <DropdownMenuSeparator className="bg-white/8" />
                          <DropdownMenuItem onClick={() => setDeleteId(inv.id)} className="text-red-400 focus:text-red-300 focus:bg-red-500/10 gap-2 cursor-pointer">
      ...
      {filtered.length > PER_PAGE && (
        <div className="flex items-center justify-between text-sm text-white/40">
          ...
          <div className="flex items-center gap-1.5">
            <Button variant="ghost" size="sm" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="text-white/40 hover:text-white hover:bg-white/5 disabled:opacity-30 gap-1">
              <RiArrowLeftSLine className="w-4 h-4" /> Prev
            </Button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
              <button key={n} onClick={() => setPage(n)} className={`w-8 h-8 rounded-lg text-sm ${n === page ? 'bg-[#FF0A54] text-white' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>
                {n}
              </button>
            ))}
            <Button variant="ghost" size="sm" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="text-white/40 hover:text-white hover:bg-white/5 disabled:opacity-30 gap-1">
      ...
      <AlertDialog open={!!deleteId} onOpenChange={v => !v && setDeleteId(null)}>
        <AlertDialogContent className="bg-[#0a0a0a] border-red-500/20">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={deleteInvoice} className="bg-red-500 hover:bg-red-600 text-white">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
